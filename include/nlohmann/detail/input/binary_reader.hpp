...
private:
  ...
  */
  ...
  get_ubjson_string(string_t &result, const bool get_char = true)
  {
    ...
    if(input_format != input_format_t::bjdata)
      ...
      {
        message = "expected length type specification (U, i, u, I, m, l, M, "
                  "L); last byte: 0x" +
                  last_token;
      }
    return sax->parse_error(
  ...
  */
  bool
  get_ubjson_size_value(std::size_t &result, bool &is_ndarray,
                  ...
                  if(result == 0 ||
                     result ==
                       npos) // because dim elements shall not have zeros,
                             ...
                             // can't be npos as it is used to initialize size
                             // in get_ubjson_size_type()
                    {
                      return sax->parse_error(
                        chars_read, get_token_string(),
                        out_of_range::create(
       ...
       (size_and_type.second & (1 << 8)) != 0)
      {
        ...
        return sax->parse_error(
          ...
          parse_error::create(
            112, chars_read,
            exception_message(input_format,
                              "BJData object does not support ND-array size in "
                              "optimized format",
                              "object"),
            nullptr));
      }
  ...
  */
  template <typename NumberType>
  bool
  get_string(const input_format_t format, const NumberType len,
             string_t &result)
  {
    bool success = true;
  ...
  */
  template <typename NumberType>
  bool
  get_binary(const input_format_t format, const NumberType len,
             binary_t &result)
  {
    bool success = true;
    for(NumberType i = 0; i < len; i++)
