...
public:
  ...
  */
  void
  write_ubjson(const BasicJsonType &j, const bool use_count,
      ...
      case value_t::array:
        {
          ...
          if(use_count)
            {
              oa->write_character(to_char_type('#'));
              write_number_with_ubjson_prefix(j.m_data.m_value.array->size(),
                                              true, use_bjdata);
            }
      ...
      case value_t::binary:
        {
          ...
          if(use_count)
            {
              oa->write_character(to_char_type('#'));
              write_number_with_ubjson_prefix(j.m_data.m_value.binary->size(),
                                              true, use_bjdata);
            }
      ...
      case value_t::object:
        {
          ...
          if(use_count)
            {
              oa->write_character(to_char_type('#'));
              write_number_with_ubjson_prefix(j.m_data.m_value.object->size(),
                                              true, use_bjdata);
            }
...
private:
  ...
  */
  void
  write_bson_array(const string_t                        &name,
  ...
  */
  bool
  write_bjdata_ndarray(const typename BasicJsonType::object_t &value,
                       const bool use_count, const bool use_type)
  {
    ...
    if(dtype == 'U' || dtype == 'C')
    ...
    else if(dtype == 'D')
      {
        for(const auto &el : value.at(key))
          {
            write_number(static_cast<double>(el.m_data.m_value.number_float),
                         true);
          }
      }
    return false;
